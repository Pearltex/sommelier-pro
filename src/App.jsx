...
import { 
  ...
  Minus, Copy, Clock, Heart, ArrowUpDown, ArrowDownAZ, FileSpreadsheet, Tag
} from 'lucide-react';
...
const FLAVOR_TAGS = ["Fruttato", "Floreale", "Minerale", "Speziato", "Erbaceo", "Tostato", "Etereo", "Dolce", "Tannico", "Fresco", "Sapido", "Caldo"];
...
const getItemStyle = (type) => {
    ...
    if (t.includes("boll") || t.includes("spumante")) return "bg-amber-50 border-amber-200 text-amber-900 dark:bg-amber-900/20 dark:border-amber-900/50 dark:text-amber-200";
    ...
    if (t.includes("birra")) return "bg-orange-50 border-orange-200 text-orange-900 dark:bg-orange-900/20 dark:border-orange-900/50 dark:text-orange-200";
    if (t.includes("spirit") || t.includes("distillato")) return "bg-slate-50 border-slate-200 text-slate-900 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300";
...
function App() {
    ...
    const saveSession = (final) => {
        if(final.mode === 'Acquisto') {
            // Se è Wishlist, q=0 o flag wishlist. Qui usiamo un flag.
    ...
    // CSV EXPORT
    const exportCSV = () => {
        let csvContent = "\uFEFFData,Vino,Produttore,Tipologia,Prezzo,Voto,Note\n"; // BOM per Excel
        logs.forEach(l => {
            l.items.forEach(i => {
                csvContent += `"${l.date}","${i.wine}","${i.prod}","${i.type}","${i.price || ''}","${i.voteWine || ''}","${l.note || ''}"\n`;
            });
        });
    ...
    const exportBackup = () => {
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify({ logs, cellar, version: "18.0" }));
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pb-24 font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
            <div className="bg-white dark:bg-slate-900 sticky top-0 z-50 px-4 py-4 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center shadow-sm">
                ...
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
                    <Card className="w-full max-w-sm animate-in zoom-in-95">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-800 space-y-2">
                            ...
                            <button onClick={exportBackup} className="w-full p-3 bg-gray-50 dark:bg-slate-800 rounded-xl flex items-center gap-2 font-bold text-sm text-slate-600 dark:text-slate-400"><Icons.DownloadCloud size={16} /> Backup JSON</button>
                            <label className="w-full p-3 bg-gray-50 dark:bg-slate-800 rounded-xl flex items-center gap-2 font-bold text-sm text-slate-600 dark:text-slate-400 cursor-pointer"><Icons.UploadCloud size={16} /> Ripristina JSON<input type="file" hidden accept=".json" onChange={importBackup} /></label>
                        </div>
                    </Card>
                </div>
            )}
            ...
            {(!session || tab !== 'session') && (
                <nav className="fixed bottom-0 left-0 w-full bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-800 pb-safe pt-2 flex justify-around items-center z-50 h-16">
            ...
            <div className="bg-slate-900 dark:bg-indigo-900 rounded-2xl p-6 text-white shadow-xl shadow-slate-300 dark:shadow-none relative overflow-hidden">
                <div className="relative z-10">
                    ...
                    <div className="flex gap-3 mt-4">
                        <div onClick={() => setTab('history')} className="bg-slate-800/50 dark:bg-black/30 px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2 cursor-pointer hover:bg-slate-800/70 active:scale-95 transition-all"><Icons.Wine size={12} className="text-pink-500"/> {logs.length} Eventi</div>
                        <div onClick={() => setTab('cantina')} className="bg-slate-800/50 dark:bg-black/30 px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-2 cursor-pointer hover:bg-slate-800/70 active:scale-95 transition-all"><Icons.Archive size={12} className="text-emerald-500"/> {cellar.length} Bottiglie</div>
                    </div>
                </div>
            </div>
...
function SessionManager({ session, setSession, onSave, onCancel, apiKey }) {
    ...
    const showEffervescence = (type) => { const t = (type || "").toLowerCase(); return t.includes("bianco") || t.includes("bollicine") || t.includes("spumante"); };
    ...
    if (step === 'adding') return (
        <div className="space-y-4 animate-in slide-in-from-right-8 fade-in pb-20">
            ...
            {session.mode !== 'Degustazione' && session.mode !== 'Acquisto' && (
                <Card className="bg-orange-50/50 dark:bg-orange-900/20 border-orange-100 dark:border-orange-900/30">
                    ...
                    <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-orange-100 dark:border-orange-900/30">
                        <label className="text-[10px] font-bold text-orange-400 uppercase mb-3 block text-center">Sommelier: Configura Ordine</label>
                        <div className="flex gap-2 overflow-x-auto pb-2 mb-2 justify-center border-b border-gray-100 dark:border-slate-800"><CounterBtn type="Rosso" label="Rossi" /><CounterBtn type="Rosato" label="Rosati/Cer." /><CounterBtn type="Bianco" label="Bianchi" /><CounterBtn type="Bollicine" label="Bolle" /></div>
                        <div className="flex gap-2 overflow-x-auto pb-2 mb-3 justify-center"><CounterBtn type="Birra" label="Birre" /><CounterBtn type="Spirit" label="Superalc." /></div>
            ...
            <Card className="border-l-4 border-l-indigo-500">
                <div className="flex gap-2 items-end"><div className="flex-1"><Input label="Etichetta / Nome" value={item.wine || ''} onChange={e => setItem({...item, wine: e.target.value})} /></div><button onClick={handleWineFill} disabled={isAiLoading} className="mb-3 p-3 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl shadow-lg shadow-purple-200 disabled:opacity-50">{isAiLoading ? <Icons.Loader2 size={20} className="animate-spin"/> : <Icons.Sparkles size={20}/>}</button><div className="mb-3"><input type="file" ref={fileInputWine} hidden accept="image/*" onChange={(e) => handlePhoto(e, 'imgWine')} /><button onClick={() => fileInputWine.current.click()} className="p-3 rounded-xl border bg-gray-50 dark:bg-slate-800 text-gray-400"><Icons.Camera size={24}/></button></div></div>
                {item.imgWine && <div className="h-40 w-full bg-contain bg-no-repeat bg-center rounded-lg mt-2 mb-4 bg-gray-100 dark:bg-slate-800" style={{backgroundImage: `url(${item.imgWine})`}}></div>}
            ...
            {/* 🏷️ FLAVOR TAGS */}
            <div className="mb-4">
                <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide">Note Rapide</label>
                <div className="flex flex-wrap gap-2">
                    {FLAVOR_TAGS.map(tag => (
                        <button 
                            key={tag} 
                            onClick={() => toggleFlavor(tag)}
                            className={`px-3 py-1 rounded-full text-xs font-bold border transition-all ${item.flavorTags?.includes(tag) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-slate-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-slate-700'}`}
                        >
                            {tag}
                        </button>
                    ))}
...
function CellarView({ cellar, setCellar, apiKey, startSession }) {
    ...
    const [sortMode, setSortMode] = useState('date-desc'); // 'price-asc', 'price-desc', 'alpha'

    // FILTRI AVANZATI + WISHLIST
    const filteredCellar = useMemo(() => {
        let data = cellar.filter(b => {
            const isWish = b.q === 0 || b.isWishlist;
            if (filter === 'wishlist') return isWish;
            if (isWish) return false; // Se non siamo in wishlist, nascondi i desideri
        ...
        return data.sort((a, b) => {
            ...
            return (b.id || 0) - (a.id || 0); // date-desc default
        });
    ...
    const handleSmartFill = async () => { if(!newBot.n) return; setLoading(true); try { const prompt = `Analizza vino: "${newBot.n}". JSON STRETTO: {"prod": "Produttore", "year": "Anno", "type": "Rosso/Bianco/Bollicine/Rosato", "drinkFrom": "2024", "drinkTo": "2030"}`; const data = await callGemini(apiKey, prompt); setNewBot(prev => ({ ...prev, p: data.prod, y: data.year, type: data.type, drinkFrom: data.drinkFrom, drinkTo: data.drinkTo })); } catch(e) { alert("Errore AI: " + e.message); } finally { setLoading(false); } };
...
function StatsView({ logs, cellar }) {
    const stats = useMemo(() => {
        ...
        return { totalSpent, totalValueCellar, avgPrice, avgScore };
    }, [logs, cellar]);
