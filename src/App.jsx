...
const PairingGraphInteractive = ({ values, onChange, foodName, wineName }) => {
    const size = 380; // Un po' più grande per far stare tutto
    const center = size / 2;
    const radius = 120;
    const labelRadius = radius + 45; // Distanza input dal centro

    // DEFINIZIONE ASSI (Angoli in gradi, 0 = ore 12)
    const axes = [
        // CIBO (Arancione)
        { id: 't_dolce', angle: 285, label: 'T. Dolce', type: 'food' }, 
        { id: 'grassezza', angle: 305, label: 'Grassezza', type: 'food' }, 
        { id: 'dolcezza_cibo', angle: 325, label: 'Dolcezza', type: 'food' },
        { id: 'succulenza', angle: 35, label: 'Succulenza', type: 'food' },
        { id: 'untuosita', angle: 55, label: 'Untuosità', type: 'food' },
        { id: 'speziatura', angle: 125, label: 'Speziatura', type: 'food' },
        { id: 'aromaticita', angle: 145, label: 'Aromaticità', type: 'food' },
        { id: 'persistenza_cibo', angle: 165, label: 'Persistenza', type: 'food' },
        { id: 'sapidita_cibo', angle: 215, label: 'Sapidità', type: 'food' },
        { id: 'amaro', angle: 235, label: 'Amarognolo', type: 'food' },
        { id: 'acido', angle: 255, label: 'T. Acida', type: 'food' },

        // VINO (Viola)
        { id: 'morbidezza', angle: 325, label: 'Morbidezza', type: 'wine' }, // Sovrapposto a dolcezza cibo (concord)
        { id: 'alcol', angle: 235, label: 'Alcolicità', type: 'wine' },
        { id: 'tannino', angle: 215, label: 'Tannicità', type: 'wine' },
        { id: 'sapidita_vino', angle: 125, label: 'Sapidità', type: 'wine' },
        { id: 'effervescenza', angle: 145, label: 'Effervescenza', type: 'wine' },
        { id: 'acidita', angle: 165, label: 'Acidità', type: 'wine' },
        { id: 'intensita', angle: 35, label: 'Intensità', type: 'wine' },
        { id: 'pai', angle: 55, label: 'P.A.I.', type: 'wine' },
        { id: 'dolcezza_vino', angle: 285, label: 'Dolcezza', type: 'wine' } // Sovrapposto a t. dolce (concord)
    ];

    const getPolyPoints = (type) => {
        // Filtra solo gli assi del tipo richiesto e li ordina per angolo
        const relevantAxes = axes.filter(a => a.type === type).sort((a, b) => a.angle - b.angle);
    ...
    return (
        <div id="printable-graph" className="relative w-full bg-white p-2 rounded-xl overflow-hidden select-none print:p-0">
            ...
            <div className="relative h-[400px] w-full">
                {/* INPUT OVERLAY - Posizionati sopra il grafico SVG */}
                {axes.map((ax, i) => {
                    // Calcolo posizione input box
                    const rad = (ax.angle - 90) * (Math.PI / 180);
                    ...
                    return (
                        <div key={ax.id} 
                             ...
                             style={{ left: x, top: y }}
                        >
                            <span className={`text-[8px] font-bold uppercase mb-0.5 whitespace-nowrap ${ax.type === 'food' ? 'text-orange-600' : 'text-indigo-600'}`}>{ax.label}</span>
                            <input 
                                type="number" 
                                min="0" max="10" 
                                className={`w-8 h-7 text-center text-xs font-bold border rounded shadow-sm focus:outline-none focus:ring-2 ${ax.type === 'food' ? 'border-orange-200 bg-orange-50 text-orange-800 focus:ring-orange-400' : 'border-indigo-200 bg-indigo-50 text-indigo-800 focus:ring-indigo-400'}`}
                                value={values[ax.id] || ''}
        ...
        <div className="min-h-screen bg-gray-100 dark:bg-black flex items-center justify-center font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300">
            ...
            <style>{`
                @media print {
                    ...
                    #printable-graph input { border: 1px solid black !important; color: black !important; }
                ...
                <div className="mt-4 mb-4">
                    ...
                    {aisTab === 'pairing' && (
                        <div className="mt-3 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-2xl border border-orange-100 dark:border-orange-800 animate-in slide-in-from-top-2 space-y-4">
                            ...
                            <div className="flex gap-2 mb-2"><div className="flex-1"><Input label="Piatto" value={item.food || ''} onChange={e => setItem({...item, food: e.target.value})} /></div><div className="flex-1"><Input label="Vino" value={item.wine || ''} onChange={e => setItem({...item, wine: e.target.value})} /></div></div>
                            
                            {/* IL GRAFICO SVG */}
                            <PairingGraphInteractive values={pairingValues} onChange={(k,v) => setPairingValues({...pairingValues, [k]: v})} foodName={item.food} wineName={item.wine} />
