...
const callGemini = async (apiKey, prompt, base64Image = null) => {
    ...
    // Manteniamo la versione suggerita dal tuo ingegnere
...
function App() {
    ...
    // CORREZIONE RESPONSIVE: overflow-x-hidden nel contenitore principale
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen pb-24 font-sans text-slate-800 dark:text-slate-100 transition-colors duration-300 overflow-x-hidden">
            <div className="bg-white dark:bg-slate-900 sticky top-0 z-50 px-4 py-4 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center shadow-sm h-16">
            ...
            {showSettings && (
                <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
                    {/* CORREZIONE IMPOSTAZIONI: Colori testo e sfondo espliciti */}
                    <Card className="w-full max-w-sm animate-in zoom-in-95 bg-white dark:bg-slate-900 text-slate-900 dark:text-white">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setShowSettings(false)} className="text-slate-500 dark:text-slate-300 hover:text-black dark:hover:text-white"><Icons.X size={20}/></button>
                        </div>
            ...
            {/* CORREZIONE RESPONSIVE: max-w-md mx-auto al main */}
            ...
            {(!session || tab !== 'session') && (
                // CORREZIONE MENU PC: Centrato e limitato in larghezza
